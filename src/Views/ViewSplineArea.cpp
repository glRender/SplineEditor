...
void ViewSplineArea::initializeGL()
{
    ...
    shadersPathes = {
        {"coloredShP",
         {{ShaderType::VertexShader, "data/colored.vertex"},
          {ShaderType::FragmentShader, "data/colored.frag"}}},
    };

    for (auto i : shadersPathes)
    {
        resMng->shaderPrograms().create(
            i.first, [i]() { return createShaderProgramFromFiles(i.second); });
    }
    ///
...
void ViewSplineArea::mousePressEvent(QMouseEvent * event)
{
    ...
    {
        Vec2 normDeviceCoords((float) event->pos().x() / width(),
